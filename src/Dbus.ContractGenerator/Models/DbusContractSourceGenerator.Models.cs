...
internal readonly struct DbusGeneratorConfigurationFile(string path, string content)
{
...
internal enum InterfaceMergePolicy
{
    Fail = 0,
    Warn = 1,
    MergePreferFirst = 2,
...
internal enum TypeNameCollisionPolicy
{
    Suffix = 0,
    Prefix = 1,
    Hash = 2
...
internal enum QtTypeHintUnknownBehavior
{
    Allow = 0,
    Warn = 1,
    Error = 2
...
internal sealed class QtTypeHintPolicy(
...
internal sealed class TypeNamingStrategy(
...
internal sealed class InterfaceMemberFilter(
...
internal readonly struct DbusXmlFile(string path, string content)
{
    public string Path { get; } = path;
...
internal sealed class DbusInterfaceModel(
    string sourcePath,
...
internal sealed class DbusMethodModel(
    string name,
...
internal sealed class DbusPropertyModel(
    string name,
    string signature,
    DbusType type,
    string access,
...
internal sealed class DbusSignalModel(
    string name,
...
internal sealed class DbusArgumentModel(string name, string signature, DbusType type, DbusAnnotationSet annotations)
{
    public string Name { get; } = name;
...
internal sealed class DbusAnnotationSet(ImmutableDictionary<string, string> values)
{
...
internal abstract class DbusType;

internal sealed class PrimitiveDbusType(char code) : DbusType
{
    public char Code { get; } = code;
}

internal sealed class ArrayDbusType(DbusType elementType) : DbusType
{
    public DbusType ElementType { get; } = elementType;
}

internal sealed class DictDbusType(DbusType keyType, DbusType valueType) : DbusType
{
...
internal sealed class StructDbusType(ImmutableArray<DbusType> elements) : DbusType
{
    public ImmutableArray<DbusType> Elements { get; } = elements;
}

internal sealed class DbusSignatureParseException(string signature, string message) : Exception(message)
{
    public string Signature { get; } = signature;
}
